...
export default function AdminQueries() {
  ...
  const renderQueries = (items: (StudentQuery | TeacherQuery)[], type: 'student' | 'teacher') => {
    if (items.length === 0) {
      return (
        <Card className="border-dashed">
          <CardContent className="py-12 text-center text-muted-foreground uppercase font-semibold tracking-widest text-xs">
            No {type} queries yet.
          </CardContent>
        </Card>
      );
    ...
    return items.map((query) => (
      <Card key={query.id} className={cn(
        "transition-all mb-4 rounded-2xl overflow-hidden border-none shadow-md",
              ...
              <CardTitle className="text-sm font-semibold uppercase tracking-tight">
                ...
                {type === 'student' && (
                  <Badge variant="outline" className={cn(
                    ...
                    (query as any).target_type === 'teacher' ? "border-purple-200 text-accent bg-purple-50" : "border-blue-200 text-info bg-info/10"
                  )}>
                    To: {(query as any).target_type}
                  </Badge>
                )}
              </CardTitle>
              <p className="text-xs text-muted-foreground font-bold font-medium">
                {format(new Date(query.created_at), 'MMM dd, yyyy - hh:mm a')}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {(query as any).is_pinned && <Badge variant="default" className="bg-primary text-xs font-semibold uppercase"><Pin className="w-3 h-3 mr-1" /> Pinned</Badge>}
            {query.is_public ? (
              <Badge variant="outline" className="text-info border-blue-200 bg-info/10 text-xs font-semibold"><Globe className="w-2.5 h-2.5 mr-1" /> Public</Badge>
            ) : (
              <Badge variant="outline" className="text-warning border-amber-200 bg-warning/10 text-xs font-semibold"><Lock className="w-2.5 h-2.5 mr-1" /> Private</Badge>
            )}
            <Badge variant={query.status === 'replied' ? 'secondary' : 'default'} className={cn(
              "text-xs font-semibold",
              query.status === 'replied' ? 'bg-success/10 text-success' : 'bg-orange-500 text-white'
            )}>
          ...
          {query.reply_content && (
            <div className="mt-4 bg-muted/30 p-4 rounded-2xl border-l-4 border-primary shadow-inner">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-muted-foreground font-bold uppercase ml-auto">
        ...
        <CardFooter className="justify-end gap-2 py-4 bg-primary/5 border-t border-primary/10">
          <Button variant="ghost" size="sm" className="text-destructive font-semibold text-xs" onClick={() => handleDelete(query.id, type)}>
            <Trash2 className="w-3.5 h-3.5 mr-1.5" /> Delete
          </Button>
          {query.is_public && (
            <Button variant="ghost" size="sm" className="font-semibold text-xs" onClick={() => togglePin(query.id, (query as any).is_pinned, type)}>
          ...
          <Button size="sm" className="rounded-xl h-9 font-semibold text-xs" onClick={() => {
            setSelectedQuery({
              id: query.id,
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div className="flex justify-between items-center bg-card p-8 rounded-2xl shadow-sm border border-primary/5">
        <div>
          <h1 className="text-3xl font-semibold uppercase tracking-tight text-primary">Queries Hub</h1>
          <p className="text-xs text-muted-foreground font-bold font-medium mt-1">Manage feedback from Teachers & Students</p>
        </div>
      ...
      <Tabs defaultValue="students" className="w-full">
        <TabsList className="bg-muted/50 p-1.5 rounded-xl h-14 mb-8">
          <TabsTrigger value="students" className="rounded-xl font-semibold text-xs data-[state=active]:bg-primary data-[state=active]:text-white">
            <User className="w-3.5 h-3.5 mr-2" />
            Student Queries
          </TabsTrigger>
          <TabsTrigger value="teachers" className="rounded-xl font-semibold text-xs data-[state=active]:bg-primary data-[state=active]:text-white">
        ...
        <TabsContent value="students" className="animate-in fade-in slide-in-from-bottom-2 duration-300">
          {loading ? (
             <div className="space-y-4">
               {[1, 2, 3].map(i => <div key={i} className="h-40 w-full bg-muted animate-pulse rounded-2xl" />)}
             </div>
        ...
        <DialogContent className="sm:max-w-[500px] rounded-2xl border-none shadow-2xl overflow-hidden p-0">
          <DialogHeader className="p-8 bg-primary text-white">
            <div className="flex items-center gap-3 mb-2">
              <Badge className="bg-white/20 text-white border-none font-semibold text-xs">Respond to {selectedQuery?.type}</Badge>
            </div>
            <DialogTitle className="text-2xl font-semibold uppercase tracking-tight leading-tight">Response to {selectedQuery?.name}</DialogTitle>
          </DialogHeader>
          <div className="p-8 space-y-6 bg-card">
            ...
            <div className="space-y-3">
              <Label className="text-xs font-semibold text-muted-foreground ml-2">Admin Response Message</Label>
              <Textarea 
            ...
            {selectedQuery?.type === 'student' && (
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-2xl">
                <div className="space-y-0.5">
                  <Label htmlFor="reply-public" className="cursor-pointer text-[11px] font-semibold uppercase tracking-tight">Public Response</Label>
                  <p className="text-xs text-muted-foreground font-bold font-medium">Visible to all students</p>
                </div>
          ...
          <DialogFooter className="p-8 bg-primary/5 gap-4">
            <Button variant="outline" className="flex-1 rounded-2xl h-12 font-semibold text-xs" onClick={() => setSelectedQuery(null)}>Discard</Button>
            <Button className="flex-1 rounded-2xl h-12 font-semibold text-xs shadow-lg shadow-primary/20" onClick={handleReply} disabled={!replyText.trim()}>Send Response</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
