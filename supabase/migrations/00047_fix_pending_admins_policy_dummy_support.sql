...
-- New policy: Existing admins (verified OR dummy) can manage all, and unverified admins can manage their own entry
create policy "Admins can manage pending admins"
  on public.pending_admins
  for all
  using (
    (exists (select 1 from public.profiles where id = auth.uid() and role = 'admin' and (email_verified = true OR email LIKE '%@miaoda.com'))) OR
