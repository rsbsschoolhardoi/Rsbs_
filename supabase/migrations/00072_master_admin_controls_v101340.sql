-- RPC for toggling master status
...
BEGIN
    ...
    SET is_master = p_status
    WHERE id = p_user_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Update handle_new_user to ensure is_master is persistent and not accidentally reset
...
DECLARE
    ...
    target_email text;
    is_oauth boolean;
    existing_is_master boolean;
BEGIN
    -- Normalized email: lowercase and trim
    target_email := LOWER(TRIM(NEW.email));
    
    -- Check if user already exists to preserve is_master status
    SELECT is_master INTO existing_is_master FROM public.profiles WHERE id = NEW.id;
    ...
    -- Preservation Logic: If existing_is_master is already set, keep it. 
    -- Otherwise, first user ever becomes master.
    IF existing_is_master IS NOT NULL THEN
        final_is_master := existing_is_master;
    ELSE
    ...
    VALUES (
        ...
        meta_initial_pin,
        (meta_initial_pin IS NOT NULL OR final_role != 'admin')
    )
    ON CONFLICT (id) DO UPDATE SET
        ...
        is_master = EXCLUDED.is_master, -- Update respects the preservation logic in final_is_master
