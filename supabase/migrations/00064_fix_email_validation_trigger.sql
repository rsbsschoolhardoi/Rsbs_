CREATE OR REPLACE FUNCTION validate_global_email_uniqueness()
RETURNS TRIGGER AS $$
BEGIN
  ...
  IF NEW.email IS NOT NULL THEN
    IF TG_TABLE_NAME = 'profiles' THEN
      IF EXISTS (SELECT 1 FROM public.profiles WHERE email = NEW.email AND id != NEW.id) OR
         ...
         EXISTS (SELECT 1 FROM public.parents WHERE email = NEW.email AND id != COALESCE(NEW.parent_profile_id, '00000000-0000-0000-0000-000000000000'::uuid))
      THEN
    ...
    ELSIF TG_TABLE_NAME = 'parents' THEN
      IF EXISTS (SELECT 1 FROM public.parents WHERE email = NEW.email AND id != NEW.id) OR
         EXISTS (SELECT 1 FROM public.profiles WHERE email = NEW.email AND parent_profile_id != NEW.id) OR
         EXISTS (SELECT 1 FROM public.students WHERE email = NEW.email) OR
