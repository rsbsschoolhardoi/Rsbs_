...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { username, password, is_admin, initial_pin } = await req.json()
    console.log(`Request to create/update user: ${username}, is_admin: ${is_admin}, has_pin: ${!!initial_pin}`)
    ...
    if (initial_pin) {
      user_metadata.initial_pin = initial_pin
    }
